...
Determine the upstream version (like `8.2.0`) and eventually update the
`package.json` file; the format is `8.2.0-pre`.
...
and fix them; assign them to a milestone (like `8.2.0`).
...
- add a new entry like _\* v8.2.0_
- commit with a message like _prepare v8.2.0_
...
- `npm dist-tag add @micro-os-plus/architecture-cortexm@8.2.0 latest`
- `npm dist-tag ls @micro-os-plus/architecture-cortexm`
